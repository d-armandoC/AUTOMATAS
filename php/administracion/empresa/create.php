...
if (!$mysqli = getConectionDb()) {
   ...
   $json = json_decode($requestBody, true);
    ...
    if ($result->num_rows > 0) {
        ...
        $insertSql = "INSERT INTO empresas (id_empresa,acronimo,empresa,direccion,telefono,correo)
            VALUES(?, ?, ?, ?, ?, ?)";
        if ($stmt = $mysqli->prepare($insertSql)) {
            $stmt->bind_param("isssss", utf8_decode(strtoupper($json["id"])),
                    $json["acronimo"], utf8_decode(strtoupper($json["empresa"])),
                    utf8_decode(strtoupper($json["direccion"])), $json["telefono"], $json["correo"]);
            $stmt->execute();
