describe("Ext.data.Session", function() {
    ...
    describe("record access", function() {
        ...
        describe("contains", function() {
            ...
            it("should accept the entity name", function() {
                expect(session.contains('User', 1)).toBe(true);
            });

            it("should accept the entity class", function() {
                expect(session.contains(spec.User, 1)).toBe(true);
            });

            it("should throw an exception with an unrecognized model name", function() {
                expect(function() {
                    session.contains('Luser', 1);
                }).toThrow();
            });
            ...
            it("should throw an exception creating an anonymous model", function() {
                ...
                expect(function() {
                    session.contains(Model, 1);
                }).toThrow();
            });

            it("should return true if the model exists", function() {
                expect(session.contains('User', 1)).toBe(true);
            });

            it("should return false if the record does not exist", function() {
                expect(session.contains('User', 12)).toBe(false);
            });
        });
    });
