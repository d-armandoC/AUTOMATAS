...
Ext.define('Ext.data.schema.Role', {
    ...
    getAssociatedStore: function (inverseRecord, data) {
        ...
        if (store === undefined) {
            store = inverseRecord.schema.createAssociationStore(session, me, inverseRecord);
    ...
    doGetFK: function (leftRecord, options, scope) {
        ...
        if (!done) {
            ...
            if (session) {
                ...
                if (foreignKeyId || foreignKeyId === 0) {
                    done = session.contains(cls, foreignKeyId) && !reload;
        ...
        if (done) {
            ...
            if (typeof options === 'function') {
                ...
                options.scope = scope || options.scope || leftRecord;
            }
