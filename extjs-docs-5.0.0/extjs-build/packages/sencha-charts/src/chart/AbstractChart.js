...
Ext.define('Ext.chart.AbstractChart', {
    ...
    constructor: function (config) {
        ...
        if (me.sprites) {
            ...
            chartSurface.setFlipRtlText(me.getInherited().rtl);
            chartSurface.add(me.sprites);
        }
    ...
    // Converts page coordinates into chart's mainRect coordinates,
    // which represent a displacement from the origin that is always located
    // at the top left corner of the mainRect, regardless of RTL mode.
    getEventXY: function (e) {
        var me = this,
            isRtl = me.getInherited().rtl,
            pageXY = e.getXY(), // Event position in page coordinates.
            xy = me.innerElement.getXY(), // Surface container position in page coordinates.
            // 'main' surface position ('mainRect') in surface container in LTR coordinates.
            mainRect = me.getMainRect() || [0,0,0,0],
            chartWidth = isRtl && me.innerElement.getWidth();
        return [
            isRtl ?
                // The line below is actually a simplified form of
                // mainRect[2] - (pageXY[0] - xy[0] - (chartWidth - (mainRect[0] + mainRect[2]))).
                xy[0] - pageXY[0] - mainRect[0] + chartWidth :
                pageXY[0] - xy[0] - mainRect[0],
            pageXY[1] - xy[1] - mainRect[1]
        ];
    },
